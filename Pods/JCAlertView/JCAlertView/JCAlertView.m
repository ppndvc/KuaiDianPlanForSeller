...
@property (nonatomic,strong)NSMutableArray *customViewButtonAction;
...
- (instancetype)initWithCustomView:(UIView *)customView  ButtonType:(JCAlertViewButtonType)buttonType ButtonTitle:(NSString *)buttonTitle Click:(clickHandle)click ButtonType:(JCAlertViewButtonType)buttonType1 ButtonTitle:(NSString *)buttonTitle1 Click:(clickHandle)click1 dismissWhenTouchedBackground:(BOOL)dismissWhenTouchBackground
{
    if (customView)
    {
        CGRect customFrame = customView.frame;
        CGRect contentFrame = customView.frame;
        
        if (customFrame.size.height > (JCScreenHeight * 0.8 - 3*JCMargin - JCButtonHeight))
        {
            customFrame.size.height = (JCScreenHeight * 0.8 - 3*JCMargin - JCButtonHeight);
            contentFrame.size.height = JCScreenHeight * 0.8;
        }
        else
        {
        ...
        {
            _customViewButtonAction = [NSMutableArray new];

            UIView *contentView = [[UIView alloc] initWithFrame:contentFrame];
            contentView.backgroundColor = [UIColor whiteColor];
            [contentView addSubview:customView];
            
            UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(JCMargin, contentFrame.size.height - JCButtonHeight - JCMargin, (contentFrame.size.width - 3*JCMargin)/2.0, JCButtonHeight)];
            
            [btn setTitle:buttonTitle forState:UIControlStateNormal];
            [self setButton:btn BackgroundWithButonType:buttonType];
            btn.tag = 0;
            [_customViewButtonAction addObject:click];

            [contentView addSubview:btn];
            
            [btn addTarget:self action:@selector(onTapButon:) forControlEvents:UIControlEventTouchUpInside];
            
            UIButton *btn1 = [[UIButton alloc] initWithFrame:CGRectMake(btn.frame.origin.x + btn.frame.size.width + JCMargin, contentFrame.size.height - JCButtonHeight - JCMargin, (contentFrame.size.width - 3*JCMargin)/2.0, JCButtonHeight)];
            [btn1 setTitle:buttonTitle1 forState:UIControlStateNormal];
            [self setButton:btn1 BackgroundWithButonType:buttonType1];
            [contentView addSubview:btn1];
            [btn1 addTarget:self action:@selector(onTapButon:) forControlEvents:UIControlEventTouchUpInside];
            btn1.tag = 1;
            [_customViewButtonAction addObject:click1];
...
-(void)onTapButon:(UIButton *)btn
{
    if (_customViewButtonAction && _customViewButtonAction.count > btn.tag)
    {
        clickHandle handler = _customViewButtonAction[btn.tag];
